...
class PropertyService {
  ...
  Future<String?> createProperty(Property property) async {
    try {
      ...
      if (e.toString().contains('NOT_FOUND') &&
        ...
        throw Exception(
          'Firebase database not configured. Please contact support.',
        );
  ...
  Future<List<Property>> getUserProperties() async {
    try {
      ...
      final properties =
          querySnapshot.docs.map((doc) => _documentToProperty(doc)).toList();
  ...
  Future<List<Property>> getAllPropertiesForDebugging() async {
    try {
      ...
      print(
        'PropertyService: Found ${querySnapshot.docs.length} total properties',
      );
      ...
      final properties =
          querySnapshot.docs.map((doc) {
            final data = doc.data();
            print(
              'Property: ${data['name']}, Status: ${data['status']}, Type: ${data['type']}',
            );
  ...
  Future<List<Property>> getApprovedProperties({int limit = 20}) async {
    try {
      ...
      print(
        'PropertyService: Found ${querySnapshot.docs.length} approved properties',
      );

      // If no approved properties found, get all properties
      if (querySnapshot.docs.isEmpty) {
        print(
          'PropertyService: No approved properties found, fetching all properties...',
        );
        querySnapshot = await _firestore.collection('properties').get();
        ...
        for (var doc in querySnapshot.docs.take(3)) {
          final data = doc.data();
          print(
            'Property: ${data['name']}, status: ${data['status']}, ownerId: ${data['ownerId']}',
          );
        }
      }
  ...
  Future<List<Property>> getMoreApprovedProperties({
    ...
    try {
      ...
      return querySnapshot.docs.map((doc) => _documentToProperty(doc)).toList();
    } catch (e) {
  ...
  Stream<List<Property>> getApprovedPropertiesStream({int limit = 20}) {
    return _firestore
        ...
        .map((snapshot) {
          // Sort locally after fetching to avoid composite index requirement
          final properties =
              snapshot.docs.map((doc) => _documentToProperty(doc)).toList();
  ...
  Future<List<Property>> searchApprovedProperties({
    ...
    try {
      ...
      final nameResults =
          nameQuery.docs.map((doc) => _documentToProperty(doc)).toList();

      final addressResults =
          addressQuery.docs.map((doc) => _documentToProperty(doc)).toList();
